...
# ==============================================================================
# 3. Moteur IA (Adresse Officielle corrigée)
# ==============================================================================
SYSTEM_PROMPT = """
...
Règles : 1. JSON valide. 2. Pas de guillemets doubles (") dans les textes. 3. Pas de HTML.
FORMAT JSON :
{{
  "questions": [
    {{
      "type": "QCU",
      "question": "Énoncé textuel",
      "options": {{"A": "A", "B": "B", "C": "C", "D": "D", "E": "E"}},
      "reponse_correcte": "A", 
      "explication": ["Explication..."], 
      "indice": "Indice...", "mnemotechnique": "Astuce..."
    }},
    {{
      "type": "OUVERTE",
      "question": "Énoncé de la question",
      "reponse_attendue": "Réponse idéale",
      "mots_cles": ["mot1", "mot2"],
      "explication": ["Rappel..."],
...
# ==============================================================================
# 4. Interface Graphique (Reste identique)
# ==============================================================================
with st.sidebar:
...
st.title("🎓 Simulateur LAS 1 (Connexion Stable)")
...
if 'data' in st.session_state:
    ...
    with t1:
        ...
        for i, q in enumerate(liste_questions):
            ...
            if type_q == "QCU":
                opts = q.get('options', {})
                choix = st.radio("Choisis :", options=list(opts.keys()), format_func=lambda x: f"{x}. {nettoyer_question(opts.get(x, ''))}", index=None, key=f"qcu_{i}", disabled=is_disabled)
            
            elif type_q == "OUVERTE":
                reponse_ouverte = st.text_area("✍️ Réponse :", key=f"ouv_{i}", height=120, disabled=is_disabled)
            ...
            if is_disabled:
                if type_q == "QCU":
                    ...
                    if juste:
                        score_qcu += 1
                        st.markdown(f"<div class='correct-box'>✅ <b>VRAI !</b></div>", unsafe_allow_html=True)
                    else:
                        st.markdown(f"<div class='error-box'>❌ <b>FAUX !</b> Attendu : {bonne_rep}</div>", unsafe_allow_html=True)
                        ajouter_erreur_session(matiere, question_propre, str(choix), bonne_rep, assembler_texte_html(q.get('explication')))
                    with st.expander("Explication"): st.markdown(assembler_texte_html(q.get('explication')), unsafe_allow_html=True)
                elif type_q == "OUVERTE":
                    ...
                    if len(mots_trouves) / max(1, len(mots_cles)) >= 0.5:
                        st.markdown(f"<div class='correct-box'>✅ <b>Bonne piste</b> ({len(mots_trouves)}/{len(mots_cles)} mots clés).</div>", unsafe_allow_html=True)
                    else:
                        st.markdown(f"<div class='warning-box'>⚠️ <b>Incomplet</b> ({len(mots_trouves)}/{len(mots_cles)} mots clés).</div>", unsafe_allow_html=True)
                    with st.expander("Correction"): st.success(f"**Attendu :**<br>{nettoyer_question(q.get('reponse_attendue'))}")
            st.divider()
            
        if is_disabled:
            if total_qcu > 0: st.metric("🎯 Note", f"{(score_qcu/total_qcu)*20:.1f} / 20")
