    ...
    try:
        ...
        if derniere_accolade != -1:
            try:
                ...
                st.toast("🛡️ La fin du document a été coupée par sécurité, mais le début est sauvé.", icon="🛡️")
                return donnees
            except:
                pass
...
{{
  "questions": [
    {{
      "type": "QCU",
      "question": "Énoncé textuel fluide sans HTML",
...
def generer_donnees(texte_pdf, texte_word, matiere, difficulte, nombre_qcm, est_mode_examen, api_key):
    ...
    # L'adresse exacte et complète reconnue par Google pour le modèle 1.5
    url_base = "https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent"
...
with st.sidebar:
    ...
    mode_examen = st.toggle("🚨 Activer le Mode Examen (Masquer les indices)")

st.title("🎓 Simulateur LAS 1 (Version Stable Originale)")
...
if f_pdf:
    ...
    st.info("💡 Attention : Limite l'analyse à des blocs de 5 à 10 pages maximum pour éviter les coupures de l'IA.")
    p_deb, p_fin = st.slider("Sélectionner les pages :", 1, p_tot, (1, min(10, p_tot)))
