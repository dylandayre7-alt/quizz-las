...
# ==============================================================================
# 4. Moteur IA Séquentiel (Stabilité Maximale - 5 Pages par lot)
# ==============================================================================
SYSTEM_PROMPT = """
...
def generer_donnees_illimitees(txt_complet, texte_word, matiere, difficulte, nombre_qcm, est_mode_examen, api_key):
    ...
    # Lot de 5 pages : c'est la configuration la plus stable.
    TAILLE_LOT = 5
    chunks_texte = []
    ...
    parametres_api = {"key": api_key.strip()}
    ...
    for idx, chunk in enumerate(chunks_texte):
        ...
        try:
            rep = requests.post(url_base, params=parametres_api, json=payload)
