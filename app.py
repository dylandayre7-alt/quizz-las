...
⚠️ RÈGLE DE SYNTAXE ABSOLUE : N'utilise JAMAIS de guillemets doubles (") dans tes textes. Utilise des guillemets simples ('). NE FAIS JAMAIS DE RETOURS A LA LIGNE DANS TON CODE JSON.
...
def generer_donnees(texte_pdf, texte_word, matiere, difficulte, nombre_qcm, est_mode_examen, api_key):
    ...
    # URL directe vers le modèle 2.5-flash
...
if f_pdf:
    ...
    if st.button("🚀 Lancer la génération", type="primary", use_container_width=True):
        if not api_key: 
            ...
            with st.spinner(f"Connexion directe à Google (Gemini 2.5 Flash) en cours..."):
                try:
                    ...
                    st.error(f"⚠️ Erreur de formatage. Essaye de réduire le nombre de pages (ex: 3-5 pages pour tester).")
            ...
            for i, q in enumerate(liste_qcm):
                ...
                if not mode_examen:
                    ...
                    if st.button(f"Vérifier Q{i+1}", key=f"v_{i}"):
                        ...
                        explication = assembler_texte(q.get('explication', ''))
                        ...
                        st.markdown(explication, unsafe_allow_html=True)
                st.divider()
